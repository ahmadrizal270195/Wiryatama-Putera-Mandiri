    ...
    <div style={{ background: COLOR.bg, minHeight: "600px", fontFamily: "ui-sans-serif, system-ui, sans-serif" }} className="flex rounded-2xl overflow-hidden min-h-screen">
      ...
      <div className="flex-1 p-6 overflow-y-auto max-h-[100vh] main-container">
        <div className="no-print">
          ...
          {tab === "stock" && (
            <StockView products={products} batches={batches} saveBatches={persist.batches} suppliers={suppliers} stockByProduct={stockByProduct} notify={notify} findName={findName} />
          )}
          {tab === "suppliers" && (
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <h2 className="text-xl font-semibold" style={{ color: COLOR.ink }}>Stok & Batch</h2>
        </div>
      ...
      <p className="text-sm mb-5" style={{ color: COLOR.inkSoft }}>
        Ribbon menunjukkan sebaran batch per produk berdasarkan urgensi expiry — hijau aman, kuning &lt;90 hari, merah &lt;30 hari / kedaluwarsa. 
        Asal produk dapat dibedakan antara <strong className="font-bold text-teal-800">Stok Opname Awal</strong> dan <strong className="font-bold text-teal-800">Pembelian Supplier</strong>.
      </p>
        ...
        <Modal title="Import Stok & Batch (Opname Awal / Pembelian)" onClose={() => setModalImport(false)} wide>
          <div className="space-y-4">
            <div className="bg-teal-50 border border-teal-200 p-3 rounded-lg text-xs text-teal-900 flex justify-between items-center">
              ...
              <Button onClick={downloadCSVTemplate} variant="secondary" className="!bg-white shrink-0">
                <Download size={14} /> Template CSV
              </Button>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <Field label="Pilih Asal Masuk Produk">
              ...
              {sourceType === "pembelian" ? (
                <Field label="Pilih Supplier / PBF Vendor">
                ...
                <Field label="Keterangan">
                  <TextInput value="Stok Awal Sistem / Stock Opname" readOnly className="!bg-gray-100 font-mono text-xs" />
                </Field>
              )}
            </div>

            <Field label="Upload File CSV Data Batch Stok">
              <input
                type="file"
                ref={fileInputRef}
                accept=".csv, .txt"
            ...
            {parsedData.length > 0 && (
              <div>
                <div className="text-xs font-semibold mb-2" style={{ color: COLOR.primary }}>
                  Preview Data Opname yang Akan Di-import ({parsedData.length} Baris):
                </div>
                <div className="max-h-56 overflow-y-auto border rounded-lg" style={{ borderColor: COLOR.border }}>
            ...
            <div className="flex justify-end gap-2 pt-3 border-t" style={{ borderColor: COLOR.border }}>
              <Button variant="ghost" onClick={() => setModalImport(false)}>Batal</Button>
              <Button onClick={submitImport} disabled={parsedData.length === 0}>
...
function PurchasesView({
  ...
  deliveryNotes, invoices, // <-- Tambahan props agar bisa cek relasi ke Penjualan
  ...
  return (
    <div>
      ...
      {subTab === "bpb" && (
        <BPBTab {...{ products, suppliers, pos, batches, pReceipts, pInvoices, deliveryNotes, saveBatches, savePOs, savePReceipts, findName, notify, getPOStatus, receivedQty }} />
      )}
      {subTab === "faktur" && (
        <FakturPembelianTab {...{ products, suppliers, pos, batches, pReceipts, pInvoices, paymentsOut, pReturns, deliveryNotes, invoices, saveBatches, savePInvoices, findName, notify, getPOStatus, pInvoiceTotal, pInvoicePaidAmount, pInvoiceReturnedAmount, pInvoiceSisa }} />
      )}
      {subTab === "retur" && (
...
function BPBTab({ products, suppliers, pos, batches, pReceipts, pInvoices, deliveryNotes, saveBatches, savePOs, savePReceipts, findName, notify, getPOStatus, receivedQty }) {
  const [modal, setModal] = useState(null);
  ...
  async function cancelBPB(pr) {
    ...
    const prBatchIds = (pr.items || []).map(it => it.batchId).filter(Boolean);
    const prBatchNos = (pr.items || []).map(it => it.batchNo).filter(Boolean);

    // Cek apakah batch dari penerimaan ini sudah terjual di Penjualan (Surat Jalan)
    const isUsedInSales = (deliveryNotes || []).some((dn) => 
      (dn.items || []).some((it) => 
        (it.allocations || []).some((a) => prBatchIds.includes(a.batchId) || prBatchNos.includes(a.batchNo))
      )
    );

    if (isUsedInSales) {
      return notify("Gagal membatalkan: Barang dari Penerimaan (BPB) ini sudah terpakai di transaksi Penjualan. Batalkan transaksi Penjualan terlebih dahulu.", "danger");
    }
    ...
    await saveBatches(working.filter(b => b.qty > 0));
...
function FakturPembelianTab({ products, suppliers, pos, batches, pReceipts, pInvoices, paymentsOut, pReturns, deliveryNotes, invoices, saveBatches, savePInvoices, findName, notify, getPOStatus, pInvoiceTotal, pInvoicePaidAmount, pInvoiceReturnedAmount, pInvoiceSisa }) {
  ...
  async function cancelInvoice(inv) {
    try {
      ...
      // 3. Kumpulkan semua Batch ID & Batch No yang diciptakan dari Pembelian ini
      ...
      // 6. Jika aman (belum pernah dijual), potong/hapus stok batch terkait
      if (inv.isDirect) {
        let working = (batches || []).map((b) => ({ ...b }));
      ...
      notify("Terjadi kesalahan saat membatalkan faktur", "danger");
    }
  }
